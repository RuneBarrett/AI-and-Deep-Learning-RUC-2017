...
void mousePressed() {
  ...
  if (mouseX > width*btnStartPos+btnWidth && mouseX < width*btnStartPos+btnWidth*2 && mouseY > height-btnHeight) {
    for (int i = 0; i < 1000; i++) {
      ...
      //print(drawStats(_input, _output, true));
    ...
    println("err: "+cumulativeError/stats.size());
    statStr = "Testing the network with 1000 random input pairs,\n yielding a cumulative accuracy error of "+nf(cumulativeError/stats.size(), 0, 2) +"%";
    //text("Testing the network with 1000 random input pairs,\n yielding a cumulative accuracy error of "+nf(cumulativeError/stats.size(), 0, 2) +"%", width*0.1, height*0.8);
    //netUpdate();
    //draw = true;
  }
